...
# SprawdŸ czy jesteœmy w repozytorium Git
if (-not (Test-Path ".git")) {
    Write-Host "? B³¹d: Nie jesteœ w katalogu g³ównym repozytorium Git!" -ForegroundColor Red
    exit 1
}

# Œcie¿ka do katalogu hooks
$hooksDir = ".git\hooks"
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Napraw b³êdy i spróbuj ponownie." -ForegroundColor Yellow
    Write-Host "Aby pomin¹æ ten hook, u¿yj: git commit --no-verify`n" -ForegroundColor Gray
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "`nPush zablokowany. Napraw b³êdy i spróbuj ponownie." -ForegroundColor Yellow
    Write-Host "Aby pomin¹æ ten hook, u¿yj: git push --no-verify`n" -ForegroundColor Gray
    exit 1
}
Write-Host "   ? Build sukces" -ForegroundColor Green
...
Write-Host "`n? Wszystkie sprawdzenia przesz³y. Push dozwolony.`n" -ForegroundColor Green
exit 0
'@
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "? SprawdŸ projekt - mog¹ wyst¹piæ konflikty`n" -ForegroundColor Yellow
}
'@
...
if (-not (Test-Path $buildLogsDir)) {
    ...
    Write-Host "   ? Katalog ju¿ istnieje: $buildLogsDir" -ForegroundColor Gray
}
...
# === Podsumowanie ===
Write-Host ""
Write-Host "=== Instalacja zakoñczona ===" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "U¿ycie:" -ForegroundColor Cyan
Write-Host "  - Hooki dzia³aj¹ automatycznie przy git commit/push/merge" -ForegroundColor White
Write-Host "  - Aby pomin¹æ hook: git commit --no-verify" -ForegroundColor White
